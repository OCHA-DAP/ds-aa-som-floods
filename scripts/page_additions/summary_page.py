...
add("<p class=\"note\">Readiness activates 8 to 12 days ahead, on the GloFAS ensemble forecast in all windows or on a SWALIM moderate flood risk alert for either river, and releases the mobilisation share; action activates 1 to 7 days ahead and releases the rest. Thresholds are fitted on each source's own record.</p>")
add(f"<p><b>Why action stops at 7 days.</b> Google Flood Hub, the Gu source, forecasts 7 days ahead and no further. GloFAS forecasts run longer, but the ensemble's high flows fade with lead (about {round((1 - FADE[7]) * 100):d} per cent below the day-1 forecast by day 7, {round((1 - FADE[12]) * 100):d} per cent by day 12), so the same rule seldom activates beyond a week: {BAND13['1-7']} times at 1 to 7 days against {'once' if BAND13['8-12'] == 1 else str(BAND13['8-12']) + ' times'} at 8 to 12 over the years both archives cover. Days 8 to 12 therefore carry readiness, not action. And the actions take 2 to 5 days once activated, so a week is enough time to act.</p>")
...
add(f"<p class=\"note\">Across both rivers: {len(flood_all)} flood years, {len(severe_all)} severe. Gauges are capped at bank full, so the largest floods record the same reading; a season where only one gauge crosses does not count (Gu 2021 at Belet Weyne).</p>")

add("<h2>Which source, and why</h2><p>Three global models were candidates. GEOGloWS runs 4 to 10 times too high on the Shabelle and has no forecast archive, so it drops out. Google Flood Hub and GloFAS were compared with the SWALIM gauges on their own record of the past (Google's retrospective run, GloFAS's version 5 reanalysis), using only the seasons in which the gauge reached its own 1-in-3 level, 2000 to 2023 (4 to 10 per gauge). Values are the median across the river's gauges.</p>"
    "<ul><li><b>Tracking.</b> Over every day of those seasons, the rank correlation between the model's daily flow and the gauge's daily level, allowing the model to run a few days ahead of or behind the gauge. 1: rises and falls exactly with the gauge; 0: no relation.</li>"
    "<li><b>Ranking.</b> The rank correlation between how high the model went each season and how high the gauge went: does the model order the floods by size as the gauge did. 1: the same order; 0: no relation; below 0: the wrong order.</li>"
    "</ul>")
rows = []
season_total = {}
for s in ("deyr", "gu"):
...
add("<figure><img src=\"figs/s_agreement.png\" alt=\"Agreement with the gauges in flood seasons, by window and model\"><figcaption>Tracking on the left, ranking on the right, as in the table. The ringed dot is the source the window runs on.</figcaption></figure>")
add(f"<p>GloFAS tracks the gauges far more closely in Deyr ({track_season[('deyr', 'glofas_v5')]:.2f} against {track_season[('deyr', 'google_grrr')]:.2f} for Google), and Google also over-activates there (three Juba activations with no flood) and misses the Shabelle in 2020. In Gu, Google tracks more closely on the Juba and level with GloFAS on the Shabelle, orders the floods closer to the gauges' order, and its forecasts go out before the flood where GloFAS v4's mostly go out after it (next section). Gu Shabelle reads near zero on ranking for every model because the gauge is capped at bank full in the largest floods.</p>")
...
add(f"<figure><img src=\"figs/s_activations.png\" alt=\"Flood seasons and activations by year and window\"><figcaption>Squares: gauge flood seasons, dark where severe. Dots: activations of the window's source. {n_act} activations in 25 years, all {len(severe_all)} severe seasons caught, one with no gauge flood behind it ({yl(outside)}, which SWALIM and WFP both record as a major flood). Return periods are Weibull, (years + 1) over activations.</figcaption></figure>")

d, g = fb_tot["deyr"], fb_tot["gu"]
add(f"<h2>Checked on the forecasts</h2><p>The tests above use each model's record of the past. A trigger runs on forecasts, so the historical forecasts were replayed to find the day the alert would have gone out, 1 to 7 days ahead, against the day the flood began at the gauges; either river counts. Only Google Flood Hub (2016 to 2023) and GloFAS v4 (2003 to 2023, plus Gu 2024 live) have archives. GloFAS's archive holds two issue days a week ({GLOFAS_ISSUES} a year) where Google's holds every day, so its replayed lead times are coarser by up to three days; the live GloFAS forecast is daily.</p>")
add("<figure><img src=\"figs/s_leads.png\" alt=\"Lead time of the first forecast issue meeting the rule, per flood season\"><figcaption>One row per flood season, * severe. Green: action window; pale green: readiness. Dot: first forecast issue meeting the rule; ticks: every later issue that met it. Left of the line is before the flood began, right is after.</figcaption></figure>")
add(f"<p>Deyr: GloFAS v4 activated before the flood in {d['per_model']['glofas_v4'].get('before', 0)} of {d['n']} seasons and was first in {d['head_to_head']['glofas_v4']} of the {d['n_both']} seasons both archives cover. Gu: Google was first in all {g['n_both']}; on the Shabelle it gave 10 to 13 days of warning in three of four seasons where GloFAS v4 gave 3 days once and nothing otherwise. Lead time and calibration were judged independently and agree.</p>")
...
add(f"<h2>SWALIM's alerts</h2><p>SWALIM's bulletins were first in {sw_first} of the {len(both_flag)} seasons where both SWALIM and the window's source flagged, and alone in {len(sw_only)} more. They are forward-looking and often early, so a SWALIM moderate flood risk alert for either river activates readiness. They cannot carry action: CERF needs a basis that can be backtested, and the bulletins are expert judgement, not a fixed rule.</p>")
...
add("<h2>What runs live, and what is open</h2><ul>"
    "<li><b>GloFAS version 4 runs both phases today.</b> Deyr is calibrated on version 5, which has no published forecast yet, and Gu on Google Flood Hub, which has no API access yet; version 4 stands in with levels refitted on its own record.</li>"
    "<li><b>Two gauges have stopped reporting</b>: Bardheere from 30 November 2023, Bualle from 14 March 2024.</li>"
    "<li><b>Impact years are not yet defined.</b> The benchmark is gauge levels, not people affected; 2013 and 2021 show why an impact cross-check is the next step. Each window holds three to five severe seasons, so every difference here is one or two events.</li></ul>")
add(f"<footer>Generated {datetime.date.today().isoformat()} from the trigger analysis outputs and checked against the analysis page at build time.</footer></div></body></html>")
