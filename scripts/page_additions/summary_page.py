...
ready_years, ready_by_window = set(), {}
...
# the SWALIM route: a moderate or higher flood risk statement for either river reaches readiness
# (swalim_seasons.json, the bulletin archive read season by season; it begins in 2006)
_sw = json.load(open(S / "swalim_seasons.json"))
SW_LEVELS = ["moderate", "high", "bank full"]
swalim_years = {int(x["season"].split()[1]) for x in _sw["seasons"] if x["level"] in SW_LEVELS}
SW_SPAN = set(range(2006, 2024))
assert swalim_years <= SW_SPAN, sorted(swalim_years - SW_SPAN)
ready_sw_years = swalim_years | (ready_years & SW_SPAN)
rate_ready_sw = rp_text(len(ready_sw_years), n=len(SW_SPAN))
...
FB_D, FB_G = fb_tot["deyr"], fb_tot["gu"]
...
thead tr:first-child:not(:last-child) th{border-bottom:none;padding-bottom:3px} thead tr:last-child th{padding-top:5px}
...
add("<h2>Findings</h2><ul>"
    ...
    f"<li>SWALIM's bulletins were first in {sw_first} of {len(both_flag)} seasons where both flagged. A SWALIM moderate flood risk alert reaches readiness; with that route readiness is reached in {len(ready_sw_years)} of the {len(SW_SPAN)} years {min(SW_SPAN)} to {max(SW_SPAN)} ({rate_ready_sw}), against {rate_ready} on the GloFAS forecast alone. Action needs a rule that can be backtested.</li></ul>")
...
add(f"<h2>Which source, and why</h2><p>Three global models were considered. GEOGloWS runs 4 to 10 times too high on the Shabelle and has no forecast archive, and was not taken further. Google Flood Hub and GloFAS were compared with the SWALIM gauges on their own records of the past, Google's retrospective run and GloFAS's version 5 reanalysis, using only the seasons in which the gauge reached its own 1-in-3 level between 2000 and 2023 ({rp3t.n_flood_seasons.min()} to {rp3t.n_flood_seasons.max()} seasons per gauge). Both measures are Spearman correlations, which compare ranks rather than raw values, and both are shown as the median across the river's gauges; ranking needs at least four seasons, which leaves Dollow out of it.</p>"
    ...
    "<li><b>Ranking</b> correlates how high the model went in each of those seasons with how high the gauge went. It tests whether the model orders the floods by size as the gauge did. A value of 1 means the same order, 0 means no relation, and a value below 0 means the wrong order.</li>"
    "</ul>")
rows = []
season_total = {}
for s in ("deyr", "gu"):
