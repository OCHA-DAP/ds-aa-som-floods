...
Daily GHA `monitoring.yml` at 12:00 UTC (15:00 Nairobi); step 1 waits for the day's own GloFAS issue on EWDS, retrying every 15 min until 16:00 UTC (`WAIT_FOR_ISSUE_UNTIL_UTC`) before taking the previous issue. Four steps, all from repo root:
...
4. `pipelines/export_monitoring_status.py` — `status.json` + `latest.png` to the orphan
   `monitoring-status` branch under `pages/monitoring/`; `deploy-pages.yml` (runs on `workflow_run` when the monitoring workflow completes)
...
- Dollow's Google point is the Juba main-stem gauge `hybas_1121039440` (since 2026-09-15). The
  ...
  retrospective, and the processed Google tables on blob carry that gauge since 2026-09-15. The trigger
  pages (feat/multisource-trigger) still show the old gauge's Dollow numbers until they are rebuilt.
